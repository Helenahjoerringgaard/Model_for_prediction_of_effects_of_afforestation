...
% Import the data
KorrigeretEpot = readtable("Korrigeret_E_pot_WW.xlsx", opts, "UseExcel", false);
clear opts
...
%% INTERCEPTION for Winter wheat
opts = spreadsheetImportOptions("NumVariables", 31);

% Specify sheet and range
opts.Sheet = "Winter wheat";
...
% Matrix
max_itc_WW = table2array(LAIS6(:,:));
% Interception Winter wheat

% Days to hours
max_itc_hr_WW = repelem(max_itc_WW,1,24);

% Max interception for 20 years
itc_matrix_WW = repmat(max_itc_hr_WW,20,1);

% Matrix til kollone
itc_WW = 0;
for i = 1:240
    itc_WW = [itc_WW itc_matrix_WW(i,:)];
end

itc_WW = itc_WW';

% INTERCEPTION MODEL
itc_box_WW = zeros(length(itc_WW),1);

for i=2:length(itc_box_WW)
    if regn(i)>=itc_WW(i)
        itc_box_WW(i)=itc_WW(i)-E_pot(i);
        if E_pot(i)>itc_WW(i)
            itc_box_WW(i)=0;
        end
    end
    if regn(i)<itc_WW(i) && itc_WW(i)>E_pot(i)
        itc_box_WW(i)= itc_box_WW(i-1)+regn(i)-E_pot(i);
        if itc_box_WW(i)<0
            itc_box_WW(i)=0;
        end
        if  itc_box_WW(i-1)+regn(i)>itc_WW(i)
            itc_box_WW(i)=itc_WW(i)-E_pot(i);
        end
    elseif regn(i)<itc_WW(i) && itc_WW(i)<=E_pot(i)
        itc_box_WW(i)=0;
    end
end

% Throughfall
TF_WW = zeros(length(regn),1);

for i=2:length(regn)
    if regn(i)+itc_box_WW(i-1)>itc_WW(i)
        TF_WW(i)=regn(i)+itc_box_WW(i-1)-itc_WW(i);
        if regn(i)+itc_box_WW(i-1)<itc_WW(i)
            TF_WW(i)=0;
        end
    end
...
% NY E_pot
E_pot_WW = zeros(length(E_pot),1);

for i=2:length(E_pot)
    if regn(i)+itc_box_WW(i-1)>itc_WW(i)
        E_pot_WW(i)=E_pot(i)-itc_WW(i);
        if E_pot(i) < itc_WW(i)
            E_pot_WW(i) = 0;
        end
    elseif regn(i)+itc_box_WW(i-1)<=itc_WW(i)
        E_pot_WW(i)=E_pot(i)-regn(i)-itc_box_WW(i-1);
        if E_pot(i)-regn(i)-itc_box_WW(i-1)<0
            E_pot_WW(i)=0;
        end
    end
end
...
for i = 2:length(TF_WW)
    I_deci(i) = TF_WW(i);
    
    if V_deci(i-1) < 0.5*Vdeficit_max && I_deci(i) >= E_pot_WW(i)
        E_a_deci(i) = E_pot_WW(i); 
    ...
    elseif V_deci(i-1) < 0.5*Vdeficit_max && I_deci(i) <= E_pot_WW(i)
        E_a_deci(i) = E_pot_WW(i);                          
        V_deci(i) = V_deci(i-1) + abs(I_deci(i) - E_a_deci(i));
        
    elseif  V_deci(i-1) >= 0.5*Vdeficit_max && I_deci(i) <= E_pot_WW(i)
        E_a_deci(i) =-(I_deci(i) - E_pot_WW(i)).*2.*(1-V_deci(i-1)./Vdeficit_max) + I_deci(i);
        V_deci(i) = V_deci(i-1) + abs(I_deci(i) - E_a_deci(i));
...
CA_nitrate = 137.3.*ones(length(K_deci)+1,1); 
...
% % MATRICE: HYDRAULIC CONDUCTIVITY: WATER CONTENT
...
nitratewheat = nitrate_conc_deci_year_sortet;

% AVERAGE AF INFILTRATION PAA AARSBASIS
K_WW_year = zeros(length(K_deci),1);
 for i = 1:length(K_deci)
   if mod(i,8928) == 0
      K_WW_year(i) = sum(K_deci(i-8927:i)); 
   end
 end
K_WW_year = nonzeros(K_WW_year);
...
Epot_year = zeros(length(E_pot_WW),1);
 for i = 1:length(E_pot_WW)
   if mod(i,8928) == 0
      Epot_year(i) = sum(E_pot_WW(i-8927:i)); 
   end
 end
Epot_year = nonzeros(Epot_year);

itc_WW_year = zeros(length(itc_box_WW),1);
 for i = 1:length(itc_WW)
   if mod(i,8928) == 0
      itc_WW_year(i) = sum(itc_box_WW(i-8927:i)); 
   end
 end
itc_WW_year = nonzeros(itc_WW_year);
itc_WW_year = [0; 0; itc_WW_year]; 

TF_WW_year = zeros(length(TF_WW),1);
 for i = 1:length(TF_WW)
   if mod(i,8928) == 0
      TF_WW_year(i) = sum(TF_WW(i-8927:i)); 
   end
 end
TF_WW_year = nonzeros(TF_WW_year);
%% NTRATE lEACHING KG N/YEAR*HA
leaching = nitrate_conc_deci_year_sortet.*K_WW_year.*10*(14.0067/62.0049)*0.001;
